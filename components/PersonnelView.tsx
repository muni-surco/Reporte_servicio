...
const PersonnelView: React.FC<PersonnelViewProps> = ({ data, onRefresh, isLoading }) => {
    ...
    const columnHeaderStyle = "px-4 py-3 text-left text-[13px] font-bold text-slate-400 uppercase tracking-widest border-b border-slate-200 bg-slate-50 sticky top-0 z-20 shadow-[0_1px_0_0_rgba(0,0,0,0.05)]";
    const cellStyle = "px-4 py-3 text-[13px] font-medium text-slate-700 border-b border-slate-50 bg-white";
        ...
        <div className="flex flex-col h-full gap-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="flex items-center gap-4 bg-white p-4 rounded-xl shadow-sm border border-slate-200 shrink-0">
                <div className="flex-1 flex flex-col">
                    <span className="text-[11px] font-bold text-[#004b93] uppercase tracking-wider mb-1 px-1">Buscar por</span>
                    <div className="relative">
                        ...
                        <input
                            ...
                            className="w-full pl-10 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-lg text-[13px] font-bold text-slate-700 focus:ring-2 focus:ring-blue-500 focus:outline-none transition-all uppercase placeholder:text-slate-300"
                        />
                    </div>
                </div>
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div className="flex flex-col min-w-[150px]">
                        <span className="text-[11px] font-bold text-[#004b93] uppercase tracking-wider mb-1 px-1">Rol Operativo</span>
                        <div className="relative">
                            <select
                                value={filterRole}
                                onChange={(e) => setFilterRole(e.target.value)}
                                className="w-full appearance-none bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-[13px] font-bold text-slate-700 focus:outline-none focus:border-blue-500 transition-all uppercase cursor-pointer pr-8"
                            >
                    ...
                    <div className="flex flex-col">
                        <span className="text-[11px] font-bold text-[#004b93] uppercase tracking-wider mb-1 px-1">Estado</span>
                        <div className="flex items-center gap-1 bg-slate-50 p-1 rounded-lg border border-slate-200 h-[38px]">
                            {['TODOS', 'ACTIVO', 'INACTIVO'].map(state => (
                                <button
                                    ...
                                    className={`px-3 py-1.5 rounded-md text-[11px] font-bold transition-all h-full ${filterState === state
                            ...
                            {isLoading ? (
                                <tr>
                                    <td colSpan={7} className="py-20 text-center">
                                        ...
                                        <p className="mt-4 text-[13px] font-bold text-slate-400 uppercase tracking-widest">Cargando Personal...</p>
                                    </td>
                                </tr>
                                ...
                                <tr>
                                    <td colSpan={7} className="py-20 text-center">
                                        ...
                                        <p className="text-[13px] font-bold text-slate-300 uppercase tracking-widest">No se encontraron registros</p>
                                    </td>
                                </tr>
                            ) : (
                                filteredData.map((person, idx) => (
                                    <tr key={idx} className="hover:bg-slate-50/50 transition-colors group">
                                        ...
                                        <td className={cellStyle}>
                                            <span className={`px-2 rounded text-[13px] font-bold border uppercase ${person.estado.toUpperCase() === 'ACTIVO'
